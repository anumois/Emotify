...
for Element in emotify.EmotifyList:
    ...
    for eE in Element[2].arrEmotifyTimeList:
        ...
        for DictKey in eE.HappyKeyword.keys():
            newHaDictVal = EmotifyDictEntry(EDEKey = DictKey, EDEValue = eE.HappyKeyword[DictKey], EDEContainer = HaDict)
            newHaDictVal.save()
        for DictKey in eE.SadKeyword.keys():
            newHaDictVal = EmotifyDictEntry(EDEKey = DictKey, EDEValue = eE.SadKeyword[DictKey], EDEContainer = SaDict)
            newHaDictVal.save()
        for DictKey in eE.SurpriseKeyword.keys():
            newHaDictVal = EmotifyDictEntry(EDEKey = DictKey, EDEValue = eE.SurpriseKeyword[DictKey], EDEContainer = SuDict)
            newHaDictVal.save()
        for DictKey in eE.AngerKeyword.keys():
            newHaDictVal = EmotifyDictEntry(EDEKey = DictKey, EDEValue = eE.AngerKeyword[DictKey], EDEContainer = AnDict)
            newHaDictVal.save()
        iteration = iteration + 1
